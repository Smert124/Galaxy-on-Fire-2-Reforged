...
import GoF2.Status;
import GoF2.TextBox;
import HardEngine.TouchTracker;
...
public final class ListItemWindow {
    ...
    private static ListItemWindow activeWindow = null;
	...
	private TouchTracker.TouchListener touchListener = new TouchTracker.TouchListener() {
		
		public void onTouchMove(int deltaX, int deltaY) {
            ...
            if(pitch > 768.0F) pitch = 768.0F;
            if(pitch < -256.0F) pitch = -256.0F;
        }
    };
	
	private void initTouchTracker() {
		touchTracker = new TouchTracker(touchListener);
        touchTracker.setSensitivity(2);
        touchTracker.setInvertVertical(true);
    }
    ...
    public final void updateRotation(int var1, int var2) {
		if(this.show3DShip) {
            if(!touchTracker.isTouching()) {
                ...
                this.speedYawCCW *= 0.95f;
                this.speedYawCW *= 0.95f;
                this.speedPitchDown *= 0.95f;
                this.speedPitchUp *= 0.95f;
	...
	public static void pointerPressed(int x, int y) {
        if(activeWindow != null && activeWindow.show3DShip) {
            activeWindow.touchTracker.pointerPressed(x, y);
        }
    }
    
    public static void pointerDragged(int x, int y) {
        if(activeWindow != null && activeWindow.show3DShip) {
            activeWindow.touchTracker.pointerDragged(x, y);
        }
    }
    
    public static void pointerReleased(int x, int y) {
        if(activeWindow != null && activeWindow.show3DShip) {
            activeWindow.touchTracker.pointerReleased(x, y);
            
            if(activeWindow != null) {
                activeWindow.speedYawCCW = 0.9F;
                activeWindow.speedYawCW = 0.9F;
                activeWindow.speedPitchDown = 0.9F;
                activeWindow.speedPitchUp = 0.9F;
            }
        }
    }
	...
	public final void updateCamera_(boolean var1) {
		...
		if(var1) {
            ...
            // Сброс вращения при открытии
            this.yaw = 1900.0F;
            this.pitch = 0.0F;
            this.speedYawCCW = 0;
            this.speedYawCW = 0;
            this.speedPitchDown = 0;
            this.speedPitchUp = 0;
