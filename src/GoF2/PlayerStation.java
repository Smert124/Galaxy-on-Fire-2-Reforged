...
public final class PlayerStation extends PlayerStaticFar {
   ...
   public PlayerStation(Station station) {
	  ...
	  // координаты и одновременно размеры коробок. Для каждой коробки нужно 6 значений: начальная XYZ и конечная XYZ
	  this.partsCollisions = new int[] {
      ...
      if(!Status.inAlienOrbit()) {
         arrayOfInt = FileRead.loadStationParts(station.getId(), Status.getSystem().getRace()); // загрузка частей станции с учетом расы системы, рисует полоску фракции на станции
      }

      int var3;
      if(arrayOfInt == null) { // если станция не указана, то заспавнить станцию войд. id - 3337 AEResourceManager.getGeometryResource(3337); Либо если читать нечего и раса системы будет или id системы = null;
      ...
      for(var3 = 0; var3 < this.stationParts.length; ++var3) {
		 ...
		 // построение коробки коллизии вокруг всех частей
		 ...
		 // анимация, используется для установки полоски нужного цвета для расы
	  ...
	  // тут части станций раскидываются по своим координатам
   ...
   // связано с масштабированием модели в зависимости от расстояния?
   ...
   // отображение частей станции. Если убрать - станции пропадут, но останется коллизия с невидимой точкой стыковки
   public final void appendToRender() {
      if(this.stationParts != null) {
         for(int var1 = 0; var1 < this.stationParts.length; ++var1) {
            GlobalStatus.renderer.forceRenderModel(this.stationParts[var1]);
         }
      ...
      if(var2 < this.maxPartDeflection && var2 > -this.maxPartDeflection && var3 < this.maxPartDeflection && var3 > -this.maxPartDeflection && var4 < this.maxPartDeflection && var4 > -this.maxPartDeflection && this.boundingBoxes != null) {
         for(int var5 = 0; var5 < var6.boundingBoxes.length; ++var5) {
            if(var6.boundingBoxes[var5].isPointInBounding(var2, var3, var4)) {
               var6.collidingPart = var5; // если убрать - кажется, что корабль отталкивается от станции не так сильно. Но не точно, требуются наблюдения
               return true;
            }
         }
      }
