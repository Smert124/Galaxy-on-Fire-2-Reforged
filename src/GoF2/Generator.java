		...
		if(station.getSystemIndex() == 15 && Status.getCurrentCampaignMission() < 16) { // запрет продажи кораблей в Mido (начало игры)
			...
			if(var10000 == 0) {
				...
				if(!GlobalStatus.cheat_mode) {
					...
					ship = new Ship[GlobalStatus.max_ships];
					for(int count = 0; count < ship.length; ++count) {
						ship[count] = Globals.getShips()[count].cloneBase();
						ship[count].setRace(Status.getSystem().getRace());
						ship[count].adjustPrice();
					}
				}
