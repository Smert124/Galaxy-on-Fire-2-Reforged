...
public final class PlayerEgo {
	...
	private KIPlayer autoTarget;
	private int autoTargetSearchCooldown;
	private static int AUTO_TARGET_RADIUS = 5000;
	private static int AUTO_TARGET_SEARCH_DELAY = 3000;
	private static int AUTO_BASE_SPEED = 3000;
	private static int AUTO_RANGE = 2500;
	...
	public PlayerEgo(Player var1) {
		...
		this.autoTargetSearchCooldown = 0;
		this.autoTurretEnabled = true;
	...
	public final void setGuns(Gun[] var1, int var2) {
		...
		if(!this.hasTurret) {
			...
			if(this.hasTurret) {
				...
				this.AUTO_BASE_SPEED = var8[0].getAttribute(12);
				this.AUTO_RANGE = (int)(((float)this.AUTO_BASE_SPEED / 3600.0F) * (float)(var8[0].getAttribute(13) * 250));
	...
	public void handleAutoTurret(int deltaTime) {
		
		if(!hasAutoTurret || !hasTurret || turretMode || !autoTurretEnabled) {
			
			return;
			
		}
		
		autoTargetSearchCooldown += deltaTime;
		
		if(autoTargetSearchCooldown >= AUTO_TARGET_SEARCH_DELAY) {
			
			autoTargetSearchCooldown = 0;
			findNearestEnemy();
			
		}
		
		if(autoTarget == null || autoTarget.isDead() || autoTarget.isDying() || !autoTarget.player.isActive() || autoTarget.var_5c1) {
		...
		AEVector3D targetPos = autoTarget.getPosition(new AEVector3D());
		...
		AEVector3D turretPos = turretStand.getLocalPos(new AEVector3D());
		int distance = turretPos.distanceTo(targetPos);
		
		if(distance > this.AUTO_RANGE) {
		...
		AEVector3D worldDir = new AEVector3D();
		worldDir.x = targetPos.x - turretPos.x;
		worldDir.y = targetPos.y - turretPos.y;
		worldDir.z = targetPos.z - turretPos.z;
		
		int len = worldDir.getLength();
		
		if(len == 0) return;
		
		worldDir.x = (worldDir.x * 4096) / len;
		worldDir.y = (worldDir.y * 4096) / len;
		worldDir.z = (worldDir.z * 4096) / len;
		
		Matrix shipMatrix = shipGrandGroup_.getToParentTransform();
		if(shipMatrix == null) return;
		
		Matrix invShipMatrix = new Matrix();
		shipMatrix.getInverse(invShipMatrix);
		
		AEVector3D localDir = new AEVector3D();
		invShipMatrix.transformVectorNoScale(worldDir, localDir);
		
		int localLen = localDir.getLength();
		
		if(localLen > 0) {
			
			localDir.x = (localDir.x * 4096) / localLen;
			localDir.y = (localDir.y * 4096) / localLen;
			localDir.z = (localDir.z * 4096) / localLen;
			
		}
		
		int targetYaw = AEMath.atan2(localDir.x, localDir.z);
		int horizontalDist = (int)Math.sqrt((long)localDir.x * localDir.x + (long)localDir.z * localDir.z);
		int targetPitch = -AEMath.atan2(localDir.y, horizontalDist);
		
		int maxPitchUp = 512;
		int maxPitchDown = -512;
		
		int currentYawCheck = turretStand.getEulerY();
		int yawNormalized = currentYawCheck % 4096;
		
		if(yawNormalized >= 2048) yawNormalized -= 4096;
		
		if(Math.abs(yawNormalized) >= 1024) {
		...
		if(Math.abs(yawNormalized) >= 2048) {
		...
		if(targetPitch > maxPitchUp) targetPitch = maxPitchUp;
		if(targetPitch < maxPitchDown) targetPitch = maxPitchDown;
		
		int currentYaw = turretStand.getEulerY();
		int currentPitch = turretGun.getEulerX();
		
		int yawDiff = targetYaw - currentYaw;
		if(yawDiff >= 2048) yawDiff -= 4096;
		if(yawDiff <= -2048) yawDiff += 4096;
		
		int pitchDiff = targetPitch - currentPitch;
		if(pitchDiff > 2048) pitchDiff -= 4096;
		if(pitchDiff < -2048) pitchDiff += 4096;
		
		int maxRotation = (turretRotationSpeed * deltaTime) / 100;
		if(maxRotation <= 1) maxRotation = 1;
		if(maxRotation >= 40) maxRotation = 40;
		...
		if(yawDiff != 0) {
			...
			turretStand.rotateEuler(0, yawDiff, 0);
			turretGun.rotateEuler(0, yawDiff, 0);
			
			if(turretViewCamera__ != null) {
				
				turretViewCamera__.rotateEuler(0, yawDiff, 0);
		...
		if(pitchDiff != 0) {
			...
			turretGun.rotateEuler(pitchDiff, 0, 0);
			
			if(turretViewCamera__ != null) {
				
				turretViewCamera__.rotateEuler(pitchDiff, 0, 0);
				
			}
			
		}
		
		Matrix gunMatrix = turretGun.getLocalTransform();
		
		if(gunMatrix != null) {
			
			AEVector3D gunDir = new AEVector3D(gunMatrix.getDirectionX(), gunMatrix.getDirectionY(), gunMatrix.getDirectionZ());
			int gunLen = gunDir.getLength();
			
			if(gunLen > 0) {
				
				gunDir.x = (gunDir.x * 4096) / gunLen;
				gunDir.y = (gunDir.y * 4096) / gunLen;
				gunDir.z = (gunDir.z * 4096) / gunLen;
				
			}
			
			int dot = gunDir.dot(worldDir);
			
			if(dot > 4000) {
				
				if(turretGun != null && player != null) {
					
					this.calcMatrix_.set(this.turretGun.getLocalTransform());
					this.player.playShootSound__(2, (long)deltaTime, false, this.calcMatrix_);
	...
	private void findNearestEnemy() {
		...
		if(radar != null) {
			...
			KIPlayer radarTarget = radar.getLockedEnemy();
			
			if(radarTarget != null && !radarTarget.isDead() && !radarTarget.isDying() && radarTarget.player.isActive() && radarTarget.player.enemy) {
				
				AEVector3D myPos = shipGrandGroup_.getPosition(new AEVector3D());
				AEVector3D targetPos = radarTarget.getPosition(new AEVector3D());
				
				if(targetPos != null && myPos.distanceTo(targetPos) < this.AUTO_RANGE) {
					
					autoTarget = radarTarget;
					return;
		...
		int minDist = this.AUTO_RANGE;
		AEVector3D myPos = shipGrandGroup_.getPosition(new AEVector3D());
		AEVector3D enemyPos = new AEVector3D();
		...
		for(int i = 0; i < enemies.length; i++) {
			...
			if(!enemy.player.isActive()) continue;
			
			if(!enemy.player.enemy) continue;
			
			if(enemy.var_5c1) continue;
			
			enemy.getPosition(enemyPos);
			
			if(enemyPos == null) continue;
			
			int dist = myPos.distanceTo(enemyPos);
	...
	public void setAutoTurretEnabled(boolean enabled) {
		
		this.autoTurretEnabled = enabled;
		
	}
	
	public boolean isAutoTurretEnabled() {
		
		return autoTurretEnabled && hasAutoTurret;
		
	}
	
	public KIPlayer getAutoTarget() {
	...
	public final void update(int var1, Radar var2, Hud var3, Radio var4, int var5) {
		...
		if(!this.freeze) {
			...
			if(this.hasTurret || this.hasAutoTurret) {
		...
		if(hasAutoTurret && !turretMode) {
			
			handleAutoTurret(var1);
	...
	public final void controlY(float var1) {
		...
		if(this.turretMode) {
			...
			if(var1 > 0) {
				...
				if(this.turretLift_ + var1 >= -500.0F) {
	...
	public final void draw(boolean var1, AECamera var2) {
		...
		if(GlobalStatus.developer_mode) {
			...
			if(this.hasAutoTurret) {
				...
				Font.drawString("AUTO_RADIUS: " + this.AUTO_RANGE, 150, 150, 6);
