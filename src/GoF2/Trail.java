...
public final class Trail {
	...
	public Trail(int color, int length) {
		this.mesh = AbstractMesh.newPlaneStrip(0, length, (byte)2);
		this.mesh.setTexture(AEResourceManager.getTextureResource(1095));
		this.mesh.setRenderLayer(2);
		this.tempVerts = new int[length * 12];
		this.vertices = new int[length * 12];
		int var3 = color << 2;
		int var4 = UV[var3 + 1];
		int var5 = UV[var3 + 3];
		this.uvs = new int[length * 8];
		
		for(length = 0; length < this.uvs.length; length += 8) {
			this.uvs[length] = UV[var3];
			this.uvs[length + 1] = var4;
			this.uvs[length + 2] = UV[var3 + 2];
			this.uvs[length + 3] = var4;
			this.uvs[length + 4] = UV[var3 + 2];
			this.uvs[length + 5] = var5;
			this.uvs[length + 6] = UV[var3];
			this.uvs[length + 7] = var5;
