...
public final class Gun {
	...
	public int calculateSpread(int type, int range, int speed, int reloadTime) {
		
		switch(type) {
			
			case 2: // MACHINE GUN
			return calculateMachineGunSpread(range, speed, reloadTime);
			
			case 3: // THERMO
			return calculateThermoSpread(range, speed, reloadTime);
	...
	public int calculateMachineGunSpread(int range, int speed, int reloadTime) {
		
		int baseSpread = 30;
		int rangeFactor = (range / 1000) * 15;
		int speedFactor = (speed / 10000) * 8;
		int fireRateFactor = reloadTime / 20;
		int spread = baseSpread + rangeFactor + speedFactor + fireRateFactor;
		return AEMath.min(AEMath.max(spread, 20), 500);
		
	}
	
	public int calculateThermoSpread(int range, int speed, int reloadTime) {
		
		int baseSpread = 200;
		int rangeFactor = (range / 1000) * (range / 1000) * 300;
		int speedFactor = (speed / 3000) * 300;
		int fireRateFactor = reloadTime / 2;
		int spread = baseSpread + rangeFactor + speedFactor + fireRateFactor;
		return AEMath.min(AEMath.max(spread, 100), 3000);
